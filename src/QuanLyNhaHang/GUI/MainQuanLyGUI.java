...
public class MainQuanLyGUI extends JFrame {
    ...
    final Color clLeftItem = new Color(179, 109, 137);
    final Color clLeftItemHover = new Color(255, 166, 203);
    final Color clLeftItemSelected = new Color(255, 158, 197);
    ...
    private void addControls() {
        ...
        pnTitle.setBackground(new Color(179, 109, 137));
        ...
        btnChat = new JLabel(new ImageIcon("image/ManagerUI/chat-icon.png"));
        btnChat.setToolTipText("Chat");
        btnChat.setBounds(46, 0, 46, 46);
        btnChat.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        pnTitle.add(btnChat);

        JLabel lblTitleText = new JLabel(new ImageIcon("image/ManagerUI/title-texttt.png"));
        ...
        JLabel lblAvatar = new JLabel(new ImageIcon("image/ManagerUI/avatarr.png"), JLabel.CENTER);
    ...
    private void addEvents() {
        ...
        btnChat.addMouseListener(new MouseListener() {
            ...
            public void mouseEntered(MouseEvent e) {
                ...
                btnChat.setBackground(clLeftItemHover);
            }

            @Override
            public void mouseExited(MouseEvent e) {
                btnChat.setOpaque(false);
                btnChat.setBackground(new Color(0, 0, 0, 0));
            }
        });
        ...
        for (JLabel lbl : listMenuLeft) {
            lbl.addMouseListener(new MouseListener() {
                ...
                public void mouseClicked(MouseEvent e) {
                    ...
                    String cardName = "";
                    if (lbl == lblBanHang) cardName = "1";
                    else if (lbl == lblKhuyenMai) cardName = "2";
                    else if (lbl == lblNhapHang) cardName = "3";
                    else if (lbl == lblSanPham) cardName = "4";
                    else if (lbl == lblNhanVien) cardName = "5";
                    else if (lbl == lblKhachHang) cardName = "6";
                    else if (lbl == lblThongKe) cardName = "7";
    ...
    private void openChatWindow() {
        String username = DangNhapBUS.taiKhoanLogin != null ? DangNhapBUS.taiKhoanLogin.getTenDangNhap() : null;
        String role = DangNhapBUS.taiKhoanLogin != null ? DangNhapBUS.taiKhoanLogin.getQuyen() : null;

        if (username == null || username.trim().isEmpty() || role == null || role.trim().isEmpty()) {
            System.out.println("Error: Invalid username or role - Username: " + username + ", Role: " + role);
            JOptionPane.showMessageDialog(this, "Vui lòng đăng nhập để sử dụng chat!");
            return;
        }
        // Map role to accepted values for chat
        if (!role.equals("admin") && !role.equals("employee")) {
            System.out.println("Invalid role, defaulting to employee: " + role);
            role = "employee";
        }
        new ChatGUI(username, role).setVisible(true);
    }
}